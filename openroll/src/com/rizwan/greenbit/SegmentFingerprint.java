...
public class SegmentFingerprint extends Thread {
    ...
    public void segmentFP() throws Exception {
        ...
        if (fingerCount > 1) {
            rv = nfseglib.segment_finger_prints(fpData.getImgData(), fpData.getImgWd(), fpData.getImgHt(), pbref, fingerCount, fgp, 1, 1);
        }
        ...
        if (fingerCount>1 && segPtr!=null) {
            ...
            if (rv == 0) {
                ...
                for (int i = 0; i < fingerCount; i++) {
                    ...
                    //UTIL
                    ImageAndFileProcessing.writeByteArrayToImageFile(tmp, com.rizwan.greenbit.FingerPrintForm.imageFileName + "_segment_i.BMP", segArray[i].sw, segArray[i].sh);
                    //UTIL
        ...
        else if(fingerCount==1)
        {
                ...
                segments = new FPSegment[fingerCount];
                    ...
                    segments[0] = new FPSegment();
                    segments[0].setX(0);
                    segments[0].setY(0);
                    segments[0].setW(fpData.getImgWd());
                    segments[0].setH(fpData.getImgHt());
                    segments[0].setSegErr(0);  
                    segments[0].setSegDataSz(fpData.getImgSZ());
                    segments[0].setSegData(fpData.getImgData());
